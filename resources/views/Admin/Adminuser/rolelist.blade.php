      ...
      <div class="mws-form-inline"> 
       <div class="mws-form-row"> 
        ...
        <div class="mws-form-item clearfix"> 
         <h4>当前用户:<span style="color:red;">{{$info->name}}</span>的角色信息</h4> 
         <ul class="mws-form-list inline">
          @foreach($role as $row)
          <li><input type="radio" name="rid[]" value="{{$row->id}}" @if(in_array($row->id,$rids)) checked @endif ><label>{{$row->name}}</label></li>
          @endforeach
        </ul> 
        </div> 
       </div> 
